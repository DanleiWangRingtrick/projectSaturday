...
public class WT2Realm extends AuthorizingRealm implements Realm {
    ...
    protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken token) throws AuthenticationException {
        ...
        Query query = entityManager.createQuery("select u.password from DBUsers u where u.username = '" + user + "'");
        String result = (String) query.getSingleResult();

        return new SimpleAccount(user, result.toCharArray(), WT2Realm.REALM);
    }
